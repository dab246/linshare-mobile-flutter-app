...
extension AuditLogEntryUserExtension on AuditLogEntryUser {
  ...
  String getResourceName(AuditLogEntryUser  auditLogEntryUser) {
    if (auditLogEntryUser is SharedSpaceMemberAuditLogEntry) {
      return auditLogEntryUser.resource.account.name;
    } else if (auditLogEntryUser is SharedSpaceNodeAuditLogEntry) {
      return auditLogEntryUser.resource.name;
    } else if (auditLogEntryUser is WorkGroupDocumentAuditLogEntry) {
      return auditLogEntryUser.resource.name;
    } else if (auditLogEntryUser is WorkGroupFolderAuditLogEntry) {
      return auditLogEntryUser.resource.name;
    } else if (auditLogEntryUser is WorkGroupDocumentRevisionAuditLogEntry) {
      return auditLogEntryUser.resource.name;
    }
    return auditLogEntryUser.action.name;
  }

  Map<AuditLogActionDetail, dynamic> getActionDetails(BuildContext context) {
    final clientLogAction = type.generateClientLogAction(this);
    final messageComponents = getActionMessageComponents();
    final actionTitle = type.getAuditLogMapping(context, clientLogAction, messageComponents)[AuditLogActionMessage.TITLE];
    final actionDetails = type.getAuditLogMapping(context, clientLogAction, messageComponents)[AuditLogActionMessage.DETAILS];
    return {AuditLogActionDetail.TITLE : actionTitle, AuditLogActionDetail.DETAIL : actionDetails};
  }

  String getLogTimeAndByActor(BuildContext context) {
    final logTime = creationDate.getMMMddyyyyFormatString();
    final byActor = AppLocalizations.of(context).audit_log_actor(actor.name);
